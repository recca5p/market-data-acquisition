...
Strategy and validation references are optional for a one-off discretionary advisory. If freshness limits are absent, record the actual delay and let `skills/trade-decision-guardrails/SKILL.md` use a conditional entry that the user must verify. Never invent a continuous-futures roll method.
...
`skills/trade-strategy-specification/SKILL.md` and separately measure extension from the M15
...
the calling agent applying `skills/trade-decision-guardrails/SKILL.md`. Set
`manual_advisory_data_sufficient: true` when
...
event using `skills/trade-journal-review/SKILL.md`.
...
acquisition_package:
  ...
  next_action: SEND_TO_CALLING_AGENT
```
...
Do not issue `LONG`, `SHORT`, or a speculative size. The calling agent applies
`skills/trade-decision-guardrails/SKILL.md` to evaluate the completed package.
