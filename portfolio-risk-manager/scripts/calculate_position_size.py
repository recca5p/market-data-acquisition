...
def main() -> int:
    try:
        ...
        slippage = decimal_value(
            payload, "estimated_slippage_per_unit", required=False, default="0"
        )
        ...
        price_risk_per_unit = abs(entry - stop) * value_per_price_unit
        total_risk_per_unit = price_risk_per_unit + fee + slippage + financing
        ...
        if target is not None:
            ...
            estimated_net_reward_per_unit = (
                gross_reward_per_unit - fee - slippage - financing
            )
