...
`$trade-orchestrator`, and never substitute an assumed balance, position,
...
Send the optional immutable snapshot to `$portfolio-risk-manager`. Missing snapshot data leaves indicative quantity null; it does not invalidate entry, stop, or targets.

Log status and masked identifiers through `$trade-journal-review`.
